#!/bin/bash
#SBATCH -J Llama-3.1-Swallow-8B-Instruct-v0.5-lr1e-4-predict       #ジョブ名。なんでもよい。
#SBATCH -p bacchus          #指定しなければデフォルトパーテーションはopus、[opus,luce,lepin,ausone,varuna,ganesa,budha,hestia]この中から選ぶ。
#SBATCH -o logs/swallow/%x_%j.log #標準出力先。この場合[ジョブ名]_[ジョブID].logというファイル名で保存される。
...
ADAPTER=./outputs/tokyotech-llm/Llama-3.1-Swallow-8B-Instruct-v0.5_lr0.0001/checkpoint-100
FILE_NAME=Llama-3.1-Swallow-8B-v0.5-lr1e-4

python -m scripts.predict.swallow \
  --model $MODEL \
  --config $CONFIG_PATH \
  --adapter $ADAPTER \
  --file_name $FILE_NAME \