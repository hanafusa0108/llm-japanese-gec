...
if __name__ == "__main__":
    ...
    def prompt_template(examples, tokenizer):
        ...
        for input, output in zip(examples["input"], examples["output"]):
            ...
            message = [
                ...
                {
                    "role": "user",
                    "content": f"入力文の文法的な誤りを正しい文法に修正してください。\n\n{input}",
                },
                {
                    "role": "assistant",
                    "content": output,
    ...
    def token_to_ids(example):
        encoded_example = tokenizer(
            example, max_length=1024, padding=True, truncation=True
        )
    ...
    print("デバッグ: train_dataset の形式")
    print(type(train_dataset))
    print(train_dataset[:1])  # 最初のサンプルを確認
    ...
    print(input_ids)  # トークンIDの内容を表示
    print("デバッグ: labels の内容")
    print(labels)  # ラベルの内容を表示
    
    # -100が割り当てられている部分をデコード
    masked_tokens = [token for token in labels if token == -100]
    print("デバッグ: -100が割り当てられている部分")
    print(masked_tokens)

    # -100以外の部分をデコード
    non_masked_tokens = [token for token in labels if token != -100]
    print("デバッグ: -100以外の部分")
    print(tokenizer.decode(non_masked_tokens))  # -100以外の部分をデコードして表示
