#!/bin/bash
#SBATCH -J Qwen3-0.6B-lr1e-4-predict       #ジョブ名。なんでもよい。
#SBATCH -p bacchus          #指定しなければデフォルトパーテーションはopus、[opus,luce,lepin,ausone,varuna,ganesa,budha,hestia]この中から選ぶ。
#SBATCH -o logs/Qwen/%x_%j.log #標準出力先。この場合[ジョブ名]_[ジョブID].logというファイル名で保存される。
...
ADAPTER=./outputs/Qwen/Qwen3-0.6B_lr0.0001/checkpoint-1000
FILE_NAME=Qwen3-0.6B-lr1e-4

python -m scripts.predict.qwen \
  --model $MODEL \
  --config $CONFIG_PATH \
  --adapter $ADAPTER \
  --file_name $FILE_NAME \