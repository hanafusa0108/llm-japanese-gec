#!/bin/bash
#SBATCH -J Llama-3.1-Swallow-8B-Instruct-v0.5-lr1e-4       #ジョブ名。なんでもよい。
...
RESPONSE_TEMPLATE='<|start_header_id|>assistant<|end_header_id|>' # swallowの応答テンプレート

LEARNING_RATE=1e-4

python -m scripts.train.swallow \
  --model $MODEL \
  --config $CONFIG_PATH \
