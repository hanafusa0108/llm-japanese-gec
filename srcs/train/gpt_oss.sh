#!/bin/bash
#SBATCH -J gpt-oss-20b-lr1e-4       #ジョブ名。なんでもよい。
#SBATCH -p bacchus          #指定しなければデフォルトパーテーションはopus、[opus,luce,lepin,ausone,varuna,ganesa,budha,hestia]この中から選ぶ。
#SBATCH -o logs/gptoss/%x_%j.log #標準出力先。この場合[ジョブ名]_[ジョブID].logというファイル名で保存される。
...
RESPONSE_TEMPLATE='<|start|>assistant<|channel|>final<|message|>' # gpt-ossの応答テンプレート
LEARNING_RATE=1e-4

python -m scripts.train.gpt_oss \
  --model $MODEL \
  --config $CONFIG_PATH \
